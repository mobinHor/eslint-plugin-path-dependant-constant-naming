...
const messages = {
    ...
    camelCaseValueViolation : 'The camelCase format should only be used for non-Literal values',
    snakeCaseValueViolation : 'The SCREAMING_SNAKE_CASE format should only be used for Literal values',
};
            ...
            VariableDeclarator: (node) => {
                ...
                if(rootIndex !== -1){
                    ...
                    if (!camelCaseResult && !snakeCaseResult) {
                        ...
                        if(camelCaseResult && varValueType === 'Literal'){
                            report(context, messages.camelCaseValueViolation, 'camelCaseValueViolation', {
                                node,
                            });
                        }
                        if(snakeCaseResult && varValueType !== 'Literal'){
